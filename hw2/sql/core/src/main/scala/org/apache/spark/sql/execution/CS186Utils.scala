...
object CS186Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    // IMPLEMENT ME
    var lastUdf: Expression = null
    val iter: Iterator[Expression] = expressions.iterator
    while (iter.hasNext) {
      val temp: Expression = iter.next
      if (temp.isInstanceOf[ScalaUdf]) {
        lastUdf = temp
      }
    }
    lastUdf.asInstanceOf[ScalaUdf]
  }
  ...
  def apply(
    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          // IMPLEMENT ME
          val temp: Row = input.next
          var currRow: Row = null
          val key: Row = cacheKeyProjection.apply(temp)
          if (cache.containsKey(key)) {
            currRow = cache.get(key)
          } else {
            currRow = udfProject.apply(temp)
            cache.put(key, currRow)
          }
          Row.fromSeq(preUdfProjection.apply(temp) ++ currRow ++ postUdfProjection.apply(temp))
        }
      }
    }
  }
