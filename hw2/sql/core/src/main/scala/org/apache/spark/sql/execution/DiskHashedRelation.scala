...
private[sql] class DiskPartition (
   ...
   * If the size of the partition exceeds the blockSize, the partition 
   * is spilled to disk.
   *
  ...
  def insert(row: Row) = {
    // IMPLEMENT ME
    if (inputClosed) {
      throw new SparkException("Should not be insert after closing input. Bad things will happen!")
    }
    val temp = new JavaArrayList[Row]
    temp.add(row)
    val rowSize = CS186Utils.getBytesFromList(temp).size
    if (measurePartitionSize() + rowSize < blockSize) {
        data.add(row)
    } else {
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      val nextStream: InputStream = Files.newInputStream(path)
      ...
      private[this] def fetchNextChunk(): Boolean = {
        ...
        if (chunkSizeIterator.hasNext) {
          val byteArray: Array[Byte] = CS186Utils.getNextChunkBytes(nextStream, chunkSizeIterator.next)
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    for (x <- 0 to (size - 1)) {
      diskPartitions(x) = new DiskPartition("partition_" + x, blockSize)
    }
    while (input.hasNext) {
      val tempRow: Row = input.next
      val num: Int = keyGenerator(tempRow).hashCode() % size
      diskPartitions(num).insert(tempRow)
    }
    for (x <- 0 to (size - 1)) {
      diskPartitions(x).closeInput()
    }
