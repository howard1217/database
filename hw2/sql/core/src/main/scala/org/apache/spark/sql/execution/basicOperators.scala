...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENT ME
    val relation: DiskHashedRelation = DiskHashedRelation(input, keyGenerator)
    val generator: (Iterator[Row] => Iterator[Row]) = CS186Utils.generateCachingIterator(projectList, child.output)
    val partitionIter: Iterator[DiskPartition]=relation.getIterator
    var iter: Iterator[Row] = Iterator()

    new Iterator[Row] {
      def hasNext() = {
        // IMPLEMENT ME
        iter.hasNext || fetchNextPartition()
      }
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        while (!iter.hasNext) {
          if (partitionIter.hasNext){
            iter = generator.apply(partitionIter.next.getData)
          } else {
            return false
          }
        }
